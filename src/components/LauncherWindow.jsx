 ...
 * No scroll, no overflow — all content visible at fixed size.
 */
...
const ipc = {
  ...
  close: () => window.electron?.closeOverlay?.(),
};
    ...
    >
      ...
      <div className="flex items-center justify-between px-4 py-3 border-b border-[#2A2A2D] flex-shrink-0">
        ...
        <div className="flex items-center gap-1">
          ...
          <button title="Hide" onClick={ipc.close}
            className="w-7 h-7 flex items-center justify-center rounded-lg text-[#9CA3AF] hover:text-[#F9FAFB] hover:bg-[#2A2A2D] transition-colors">
            <X size={15} />
          </button>
        </div>
      ...
      <div className="flex flex-col gap-3 px-4 py-3 flex-1">
        ...
        {/* Capture Area label */}
        <p className="text-[#9CA3AF] text-xs font-medium uppercase tracking-widest px-1 flex-shrink-0">
          Capture Area
        </p>

        {/* Capture type options — both always visible */}
      ...
      <div className="px-4 pb-4 flex-shrink-0">
        <button onClick={ipc.startCapture}
