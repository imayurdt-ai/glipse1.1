 ...
 * Full settings UI rendered inside the Launcher window.
 * Auto-saves every change to electron-store via IPC.
 * Shortcut section supports recording a new key combo.
 */

import React, { useEffect, useState, useRef } from 'react';
import {
  ArrowLeft, Monitor, Globe, Palette,
  Keyboard, FolderOpen, Save, RotateCcw,
} from 'lucide-react';
...
// Pretty-print accelerator for display
...
export default function SettingsPage({ onBack }) {
  ...
  const handleRecordKey = (e) => {
    ...
    if (!acc.includes('+')) { setShortcutError('Must include a modifier key (Ctrl / ⌘ / Alt / Shift)'); return; }
    setShortcutError('');
  ...
  const confirmShortcut = async () => {
    ...
    if (ok === false) {
      setShortcutError('Shortcut already in use by another app. Try a different combo.');
      return;
    }
          ...
          {!recording ? (
            <div className="flex items-center gap-2">
              ...
              {settings.shortcut !== 'CommandOrControl+Shift+5' && (
                <button onClick={() => { save({ shortcut: 'CommandOrControl+Shift+5' }); window.electron?.registerShortcut?.('CommandOrControl+Shift+5'); }}
            ...
            <div className="flex flex-col gap-2">
              ...
              {shortcutError && (
                <p className="text-[#EF4444] text-[10px]">{shortcutError}</p>
              )}
              <div className="flex gap-2">
                <button onClick={confirmShortcut} disabled={!shortcutDraft}
                  className="flex-1 bg-[#3B82F6] disabled:opacity-40 text-white text-xs font-semibold py-1.5 rounded-lg transition-colors hover:bg-[#2563EB]">
                  Save shortcut
                </button>
